...
export class PluginProviderSendEmailFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.loading = true;
    this.datastore.findRecord(FormModel, this.form_id).subscribe((data) => {
      this.form = data;
      this.loading = false;
    }, (error) => {
      this.loading = false;
  ...
  get body(): string {
    ...
    if (this.data.taxvat.length > 14) {
      taxvat_text = `portador do CPF <b>${this.data.taxvat_resp}</b>, responsavel pela empresa <b>${this.data.company_name}</b> registrada sob o CNPJ`
    }
    body += `<p style="text-align:center" class="text-black">
      Eu&#160;<b>${this.data.name}</b>, ${taxvat_text} &#160;<b>${this.data.taxvat}</b>, declaro que estou ciente que as
  ...
  failed() {
    ...
    this.success = false;
    this.error = "Ocorreu um erro ao enviar seu email, por favor tente novamente!";
  }
  sendEmail() {
    this.isSending = true;
