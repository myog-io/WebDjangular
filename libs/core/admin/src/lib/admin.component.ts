...
export class AdminComponent implements OnInit {
  ...
  ngOnInit() {
    this.roleProvider.parsedPermissionsEmitter.subscribe(perms => {
  ...
  getDynamicMenu() {
    this.datastore
      .findAll(CoreConfigGroupModel)
      .subscribe((data: JsonApiQueryData<CoreConfigGroupModel>) => {
        ...
        for (const m in models) {
          if (models.hasOwnProperty(m)) {
            ...
            this.menu[i].children[i2].children.push({
              ...
              hidden: false,
              data: {
                permission: [{ label: 'webdjango', action: 'view_coreconfig' }]
              }
            });
          }
        }
        ...
        if (typeof menuList[i]['data'] !== 'undefined') {
          ...
          if (typeof menuList[i].data['permission'] !== 'undefined') {
            for (let j = 0; j < menuList[i].data['permission'].length; j++) {
              ...
              const permActionName =
                menuList[i].data['permission'][j]['action'];
              //list_core_website
              if (typeof permissions[permLabelIndex] !== 'undefined') {
                //we do a OR for the permission (it must have at least 1)
                if (
                  permissions[permLabelIndex].indexOf(permActionName) === -1
                ) {
