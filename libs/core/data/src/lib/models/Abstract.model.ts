...
} from 'libs/core/builder/src/lib/interfaces/form-config.interface';
          ...
          for (let i = 0; i < children.length; i++) {
            ...
            if (belongsTo) {
              for (let j = 0; j < belongsTo.length; j++) {
                ...
                if (
                  belongsTo[j].relationship ===
                  this.modelConfig.type.toLowerCase()
                ) {
                  if (
                    !cur_child[belongsTo[j].propertyName] ||
                    cur_child[belongsTo[j].propertyName].id != entry.id
                  ) {
            ...
            child_to_remove.splice(
              child_to_remove.findIndex(
                data =>
                  data.id === cur_child.id &&
                  data.modelConfig.type === cur_child.modelConfig.type
              ),
              1
            );
                    ...
                    if (response.id) {
                      child_to_remove.splice(
                        child_to_remove.findIndex(
                          data =>
                            data.id === response.id &&
                            data.modelConfig.type === response.modelConfig.type
                        ),
                        1
                      );
                    }
          ...
          for (let i = 0; i < child_to_remove.length; i++) {
            ...
            if (del_child.id) {
              ...
              let promise = new Promise((resolve_delete, reject_delete) => {
                ...
                this.service
                  .deleteRecord(del_child.constructor, del_child.id)
                  .subscribe(
                    response => {
  ...
  public getForm(): AbstractForm {
    ...
    for (let i = 0; i < dg.length; i++) {
      for (let j = 0; j < dg[i].groups.length; j++) {
        dg[i].groups[j].fields = formFields.filter(
          field => dg[i].groups[j].name == field.displayGroup
        );
      }
    }
    fg.displayGroups = dg;
