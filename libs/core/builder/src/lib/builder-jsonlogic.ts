export class JsonLogic {
   ...
   * @param logic
   */
  private get_operator(logic): string {
   ...
   * @param logic object
   */
  private get_values(logic) {
    return logic[this.get_operator(logic)];
  }
  /**
   * return
   * @param logic Logic Json
  ...
  public apply(logic: any, data?: any) {
    ...
    var op = this.get_operator(logic);
    var values = logic[op];
    var i;
    var current;
    ...
    if (op === 'if' || op == '?:') {
      ...
      */
      for (i = 0; i < values.length - 1; i += 2) {
        if (this.truthy(this.apply(values[i], data))) {
          return this.apply(values[i + 1], data);
        }
      }
      ...
      for (i = 0; i < scopedData.length; i += 1) {
        if (!this.truthy(this.apply(scopedLogic, scopedData[i]))) {
