...
@Component({
  ...
  template: `
    <div *ngIf="input">
      ...
      >
        <div *ngIf="input.errors.required">{{ errors.error_required }}</div>
        <div *ngIf="input.errors.minlength">{{ errors.error_min_length }}</div>
        <div *ngIf="input.errors.maxLength">{{ errors.error_max_length }}</div>
        <div *ngIf="input.errors.email">{{ errors.error_email }}</div>
  ...
  ngOnInit() {
    ...
    for (const key in this.errors) {
      if (this.errors.hasOwnProperty(key)) {
        this.errors[key] = this.formatMessage(this.errors[key], {
