...
export class MenuBuilderComponent implements OnInit, OnDestroy {
  ...
  removeItem($event, item_id) {
    ...
    this.datastore.deleteRecord(MenuItemModel, item_id).subscribe(response => {
      this.current_menu.menu_item.slice(
        this.current_menu.menu_item.findIndex(item => item.id == item_id),
        1
      );
    ...
    this.current_menu.save().subscribe(
      menu => {
        ...
        if (promises.length > 0) {
          Promise.all(this.saveAllPromise()).then(menu_items => {
  ...
  createMenuItem($event) {
    ...
    if (this.current_menu.menu_item.length > 0) {
      new_item.position =
        this.current_menu.menu_item[this.current_menu.menu_item.length - 1]
          .position + 1;
    } else {
      new_item.position = 0;
    }
  ...
  public drop(e) {
    if (e.changedElementPosition) {
      const index = this.current_menu.menu_item.findIndex(
        i => i.id === e.item.id
      );
      const item: MenuItemModel = this.current_menu.menu_item[index];
      // Cleaning Parents First
      if (item.parent) {
        const parent = this.current_menu.menu_item.find(
      ...
      if (e.destination) {
        const parent: MenuItemModel = this.current_menu.menu_item.find(
          i => i.id === e.destination.id
        );
        if (!parent.children) {
          parent.children = [];
        }
