import { AfterViewInit, ChangeDetectionStrategy, ChangeDetectorRef, Component, ViewEncapsulation, Inject, } from '@angular/core';
...
@Component({
  ...
  <wda-content-viewer class="body" [content]="content" (docRendered)="renderComplete()"></wda-content-viewer>
...
export class CoreDynamicPageLoaderComponent implements AfterViewInit {
  ...
  private loadPagesContent(page: PageModel | ErrorResponse) {
    ...
    if(this.wdaConfig.current_header) this.header = this.wdaConfig.current_header.getContent();
    if(this.wdaConfig.current_footer) this.footer = this.wdaConfig.current_footer.getContent();
